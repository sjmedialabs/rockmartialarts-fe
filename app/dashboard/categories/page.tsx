...
export default function CategoriesManagementPage() {
  ...
  const handleToggleStatus = async (category: Category) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/categories/${category.id}`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
      ...
      if (response.ok) {
        ...
        if (isParentCategory && !newStatus && subcategories.length > 0) {
          const updatePromises = subcategories.map(subCat => 
            fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/categories/${subCat.id}`, {
              method: "PUT",
              headers: {
                "Authorization": `Bearer ${token}`,
                "Content-Type": "application/json"

...
export default function CoachAttendanceReportsPage() {
  ...
  const loadDropdownData = async () => {
    try {
      ...
      // Load students and courses for filtering (coach can only see their assigned students)
      const [studentsRes, coursesRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/coaches/${coachData?.id}/students`, { headers }),
        fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/courses`, { headers })
      ])
  ...
  const fetchAttendanceReports = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/attendance/reports?${params.toString()}`, {
        method: 'GET',
        headers
      })

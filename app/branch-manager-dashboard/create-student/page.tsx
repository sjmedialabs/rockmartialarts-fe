...
export default function CreateStudent() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // Load branches first, then derive locations from branches
      try {
        ...
        if (branchesResponse.ok) {
          ...
          const branchesArray = branchesData.branches || []
          ...
          if (branchesArray.length === 0) {
            ...
            toast({
              title: "Warning",
              description: "No branches found. Please contact administrator.",
              variant: "destructive",
            })
          }
          ...
          // Derive unique locations from branches using address.state field
          const statesWithNulls = branchesArray.map(branch => branch.address?.state)
          console.log('🗺️ All states (including nulls):', statesWithNulls)

          const validStates = statesWithNulls.filter(Boolean)
          console.log('🗺️ Valid states:', validStates)

          const uniqueStates = [...new Set(validStates)]
          console.log('🗺️ Unique states:', uniqueStates)

          const locationsFromBranches = uniqueStates.map(state => ({
            id: state,
            name: state,
            state: state
          }))

          setLocations(locationsFromBranches)
          console.log('✅ Locations derived from branches:', locationsFromBranches)
          console.log('📋 Sample branch structure:', branchesArray[0])

          if (locationsFromBranches.length === 0) {
            console.warn('⚠️ No valid locations derived from branches')
            toast({
              title: "Warning",
              description: "No locations found. Branch data may be incomplete.",
              variant: "destructive",
            })
          }
        } else {
          const errorText = await branchesResponse.text()
          console.error('❌ Failed to load branches:', branchesResponse.status, branchesResponse.statusText, errorText)
          toast({
            title: "Error",
            description: `Failed to load branches: ${branchesResponse.status} ${branchesResponse.statusText}`,
            variant: "destructive",
          })
        }
      } catch (error) {
        ...
        console.log('🏁 Finished loading branches and locations')
      }
  ...
  // Filter branches based on selected location (using address.state field)
  const filteredBranches = useMemo(() => {
    console.log('Filtering branches:', {
      selectedLocation: formData.location,
      totalBranches: branches.length,
      branchStates: branches.map(b => b.address?.state)
    })
    if (formData.location) {
      const filtered = branches.filter(branch => branch.address?.state === formData.location)
      console.log('Filtered branches result:', {
        filteredCount: filtered.length,
        filteredBranches: filtered.map(b => ({ name: b.name, state: b.address?.state }))
      })
      return filtered
    }
    console.log('No location selected, returning all branches:', branches.length)
    return branches
  ...
  // Filter coaches based on selected course
  useEffect(() => {
    if (formData.course) {
      // For now, show all coaches. In a real app, you might filter by course expertise
      setFilteredCoaches(coaches.filter(coach => coach.is_active))
    } else {
      setFilteredCoaches([])
    }
  }, [formData.course, coaches])
  ...
  const handleInputChange = (field: string, value: any) => {
    ...
    if (field === 'location') {
      console.log('🗺️ Location changed, clearing branch selection')
      setFormData(prev => ({ ...prev, branch: "" }))
    }
    if (field === 'category') {
      console.log('📁 Category changed, clearing course and coach selections')
      setFormData(prev => ({ ...prev, course: "", assignedCoach: "" }))
    }
    if (field === 'course') {
      console.log('📚 Course changed, clearing coach selection')
      setFormData(prev => ({ ...prev, assignedCoach: "" }))
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (requiresPayment) {
        ...
        console.log("Creating student with branch manager authentication:", apiPayload)

        // Get branch manager authentication headers
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-yellow-600 space-y-1">
                            ...
                            <p>• Branch manager will be notified of the new registration and payment</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          <SelectContent className="rounded-xl border border-gray-200 bg-white shadow-lg max-h-60">
                            {(() => {
                              console.log('🎯 Location dropdown render - locations:', locations)
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          <SelectContent className="rounded-xl border border-gray-200 bg-white shadow-lg max-h-60">
                            {(() => {
                              console.log('🏢 Branch dropdown render:', {
                                selectedLocation: formData.location,
                                filteredBranchesCount: filteredBranches.length,
                                filteredBranches: filteredBranches.map(b => ({ name: b.name, state: b.address?.state })),
                                isLoadingBranches
                              })

                              if (filteredBranches.length > 0) {
                                const validBranches = filteredBranches.filter((branch) => branch && branch.id && branch.id.trim() !== '')
                                console.log('🏢 Valid branches after filtering:', validBranches.length)

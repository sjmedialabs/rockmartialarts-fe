...
// Helper function to format address object into a readable string
...
export default function CreateStudent() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      console.log('🔍 Starting coach authentication check...')
      ...
      try {
        ...
        console.log('🔍 Coach auth result:', authResult)

        if (authResult.isAuthenticated && authResult.coach) {
          console.log('✅ Coach authenticated successfully:', {
            coachId: authResult.coach.id,
            fullName: authResult.coach.full_name,
            email: authResult.coach.contact_info?.email || authResult.coach.email,
            branchId: authResult.coach.branch_id
          })

          setCoachData(authResult.coach)
          setCoachToken(authResult.token)

          // Pre-populate branch if coach has one assigned
          if (authResult.coach.branch_id) {
            console.log('🏢 Pre-populating branch:', authResult.coach.branch_id)
            setFormData(prev => ({
              ...prev,
              branch: authResult.coach.branch_id
            }))
          }
        } else {
          // For debugging: Don't redirect immediately, just log the error
          console.log('❌ Coach not authenticated. Error:', authResult.error)
          console.log('🔄 Will redirect to /coach/login in 5 seconds...')
        ...
        // Always set auth loading to false
  ...
  useEffect(() => {
    // Only load data after authentication check is complete
    if (isAuthLoading) {
      console.log('⏳ Waiting for authentication to complete...')
      return
    }

    console.log('🔍 Authentication check complete, proceeding with data loading')
    console.log('👤 Coach data:', coachData ? 'Available' : 'Not available')
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (locationsResponse.ok) {
          ...
          console.log('❌ Failed to load locations:', locationsResponse.status)
        }
      } catch (error) {
      ...
      // Set main loading to false after all data loading attempts are complete
      console.log('✅ Data loading completed')
      setIsLoading(false)
    }

    loadData()
  }, [isAuthLoading]) // Load data after authentication check is complete
  ...
  useEffect(() => {
    const loadBranchesForLocation = async () => {
      console.log('🏢 Loading branches for location:', formData.location)

      if (!formData.location) {
        console.log('❌ No location selected, clearing branches')
        setBranches([])
        return
      }
      ...
      try {
        ...
        console.log('📡 Branches API response:', response.status)

        if (response.ok) {
          const data = await response.json()
          console.log('✅ Branches loaded:', data.branches?.length || 0, 'branches')
          console.log('📋 Branch data:', data.branches)
          setBranches(data.branches || [])
        } else {
          console.error('❌ Failed to load branches for location:', response.status, response.statusText)
          setBranches([])
          toast({
            title: "Warning",
    ...
    try {
      ...
      if (requiresPayment) {
        ...
        if (response.ok) {
          ...
          toast({
            title: "Success!",
            description: "Student created successfully with payment processing.",
            variant: "default",
          })
          // Reset form
          setFormData({
          ...
          router.push('/coach-dashboard/students')
        } else {
        ...
        {(isAuthLoading || isLoading) && (
          <div className="bg-white rounded-2xl shadow-sm border p-8 text-center">
            ...
            <p className="mt-4 text-gray-600">
              {isAuthLoading ? "Authenticating..." : "Loading form data..."}
            </p>
          </div>
        )}
            ...
            <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-8">
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-yellow-600 space-y-1">
                            ...
                            <p>• Coach will be notified of the new registration and payment</p>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-200">
                <Button
                  ...
                  variant="outline"
                  onClick={() => router.push("/coach-dashboard/students")}
                  className="flex-1 sm:flex-none px-8 py-3 text-base font-medium border-gray-300 hover:bg-gray-50 transition-all duration-200"
                >
                  Cancel
                </Button>
                <Button
                  ...
                  className="flex-1 sm:flex-none px-8 py-3 text-base font-medium bg-yellow-400 hover:bg-yellow-500 text-gray-900 transition-all duration-200"
                >
                  {isSubmitting ? (
                    <>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
              <div className="text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Student Created Successfully!</h3>
                <p className="text-gray-600 mb-6">The student has been registered and added to your dashboard.</p>
                <Button
